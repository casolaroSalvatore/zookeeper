...
package org.apache.zookeeper.server.snapshotcomparer.variant_c2;
 ...
 * configurable thresholds and filters, and outputs information about the
 * delta.
 *
...
public class SnapshotComparer {
    ...
    private void compareSnapshots(String[] args) throws Exception {
        ...
        File leftSnapshot =
                (File) parsedOptions.getParsedOptionValue(LEFT_OPTION);
        File rightSnapshot =
                (File) parsedOptions.getParsedOptionValue(RIGHT_OPTION);

        int byteThreshold = Integer.parseInt(
                (String) parsedOptions.getParsedOptionValue(BYTE_THRESHOLD_OPTION));
        int nodeThreshold = Integer.parseInt(
                (String) parsedOptions.getParsedOptionValue(NODE_THRESHOLD_OPTION));
    ...
    private static class TreeInfo {
        ...
        private static class TreeNode {
            ...
            private static class AlphabeticComparator
    ...
    /**
     * Compares the nodes of the left tree and the right tree at the same depth.
     *
     * @param left left data tree
     * @param right right data tree
     * @param depth depth of the data trees to compare
     * @param byteThreshold node data delta threshold in bytes
     * @param nodeThreshold descendant node delta threshold
     * @param debug whether detailed debug information is printed
     * @param interactive whether the comparison is running interactively
     */
    ...
    private static void compareNodes(
        ...
        Comparator<TreeInfo.TreeNode> alphabeticComparator =
                TreeInfo.makeAlphabeticComparator();

        Collections.sort(leftNodes, alphabeticComparator);
        Collections.sort(rightNodes, alphabeticComparator);
        ...
        while (leftIndex < leftNodes.size()
            ...
            if (leftIndex >= leftNodes.size()) {
                printRightOnly(
                        rightNodes.get(rightIndex),
                        byteThreshold,
                        nodeThreshold,
                        debug,
                        interactive);
            ...
            if (rightIndex >= rightNodes.size()) {
                printLeftOnly(
                        leftNodes.get(leftIndex),
                        byteThreshold,
                        nodeThreshold,
                        debug,
                        interactive);
            ...
            TreeInfo.TreeNode leftNode = leftNodes.get(leftIndex);
            TreeInfo.TreeNode rightNode = rightNodes.get(rightIndex);
            ...
            if (comparison < 0) {
                printDebugRelation(debug, "left is less");
                printLeftOnly(
                        leftNode,
                        byteThreshold,
                        nodeThreshold,
                ...
                printDebugRelation(debug, "right is less");
                printRightOnly(
                        rightNode,
                        byteThreshold,
                        nodeThreshold,
    ...
    private static void printDebugRelation(
            boolean debug,
            String relation) {
